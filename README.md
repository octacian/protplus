...
Protected regions also allow setting flags, which can change other game variables such as preventing damage or showing an HUD in the bottom-right corner of the screen. These flags can be managed via the `/protflags` chatcommand. Valid flags include `damage` (boolean).
